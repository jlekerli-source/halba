...
try {
  ...
  const trustInbox = await readFile(path.join(temporaryRoot, "trust-inbox.js"), "utf8");
  ...
  const reviewContract = await readFile(path.join(temporaryRoot, "shared", "review-contract.js"), "utf8");
  const trustContract = await readFile(path.join(temporaryRoot, "shared", "trust-contract.js"), "utf8");
  const workspaceContract = await readFile(path.join(temporaryRoot, "shared", "workspace-contract.js"), "utf8");
  ...
  assert.ok(app.includes("Trust Inbox"));
  assert.ok(app.includes("requestTrustOperations"));
  assert.ok(trustInbox.includes("export function validateTrustOperationsReport"));
  assert.ok(workspaceImport.includes('from "./shared/workspace-contract.js"'));
  assert.ok(workspaceState.includes('from "./shared/review-contract.js"'));
  assert.ok(reviewContract.includes("export function reviewDecisionMatches"));
  assert.ok(trustContract.includes("export function validateTrustConfiguration"));
  assert.ok(workspaceContract.includes("export function validateWorkspace"));
